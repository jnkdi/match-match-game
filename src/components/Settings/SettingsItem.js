const SettingsItem = (props) => {
  ...
  return (
    <div className={props.className}>
      ...
      <select id={props.title} onChange={changeParamHandler} value={props.param === 'amount' ? props.cardsAmount : props.cardsSet}>
        {props.options.map((option) => (
          <option value={option.value} key={Math.random().toString()}>{option.label}</option>
        ))}
      </select>
    </div>
  );
